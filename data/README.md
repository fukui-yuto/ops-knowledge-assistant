...
│   ├── confluence/     #   出元: Confluence
│   ├── internal/       #   出元: 社内ドキュメント
│   └── {任意の名前}/   #   出元システム名は自由に命名可能
├── ticket/             # チケット（障害対応記録・インシデント履歴）
│   └── jira/           #   出元: JIRA
...
**ルール:**
- 第1階層: `procedure`, `ticket`, `config`, `log` のいずれか（それ以外は無視）
- 第2階層: 出元システム名（自由に命名可能）

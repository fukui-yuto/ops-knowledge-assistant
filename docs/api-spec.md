...
[add]    wiki/server_setup.md → "サーバー構築手順" (5 chunks)
[update] wiki/backup_config.md → "バックアップ設定手順" (8 chunks)
[skip]   wiki/deploy_flow.md (変更なし)
[delete] issue/JIRA-789.md → DB + ChromaDB から削除
...
[OK] PostgreSQL
[OK] ChromaDB
[OK] GOOGLE_API_KEY (provider=gemini)
```
...
|---|---|---|---|
| q | string | Yes | 検索クエリ |
| source_type | string | No | フィルタ: source_type（`wiki` / `issue`） |
...
| LLM_API_ERROR | 502 | LLM API（Gemini / OpenAI）呼び出し失敗 |
